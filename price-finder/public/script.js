// public/script.js (FINAL, with admin messaging and all text changes)
...
function showFlashMessage(message, duration = 10000) {
    themeNotificationEl.innerHTML = `<b>${message}</b>`;
...
async function pollForLiveState() {
    try {
        ...
        if (data.permanentMessage) {
            permanentMessageBanner.textContent = data.permanentMessage;
            permanentMessageBanner.classList.remove('hidden');
        } else {
            permanentMessageBanner.classList.add('hidden');
        }
        
        if (data.flashMessage && data.flashMessage.timestamp > lastSeenFlashTimestamp) {
            showFlashMessage(data.flashMessage.text);
            lastSeenFlashTimestamp = data.flashMessage.timestamp;
        }
    ...
    } catch (error) { console.error("Live state poll failed:", error); }
}
...
function applyTheme(themeName, showNotification = false) {
    ...
    if (showNotification) {
        showFlashMessage(`Theme changed to: ${themeName.charAt(0).toUpperCase() + themeName.slice(1)}`, 2000);
    }
}
...
async function initializeState() {
    try {
        ...
        if (data.flashMessage) lastSeenFlashTimestamp = data.flashMessage.timestamp;
        if (data.theme) applyTheme(data.theme, false);
        if (data.rainEventTimestamp) lastSeenRainEvent = data.rainEventTimestamp;
    } catch (error) { console.error("Failed to initialize live state:", error); } 
    finally { setInterval(pollForLiveState, 5000); }
}

initializeState();

// --- ADMIN PANEL LOGIC ---
// This requires all the admin element selections from your original file
...
// ... and so on for all admin elements.
...
async function setPermanentMessage(message) {
    if (!message.trim() && !confirm("Are you sure you want to clear the permanent message?")) return;
...
async function sendFlashMessage(message) {
    if (!message.trim()) { alert("Please enter a message to send."); return; }
