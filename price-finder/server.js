...
const MAIN_PRODUCT_PRICE_THRESHOLD = 200; // Used to check if scraper only found cheap items.
...
app.post('/submit-results', async (req, res) => {
    ...
    if (allScraperResults.length > 0) {
        if (isComponentOrAccessory) {
            finalResults = allScraperResults; // For components, trust the results more
        } else {
            const maxPrice = Math.max(...allScraperResults.map(item => item.price));
            if (maxPrice < MAIN_PRODUCT_PRICE_THRESHOLD) {
                console.log(`[Fallback Trigger] Main product search for "${query}" only found cheap items (max price: $${maxPrice.toFixed(2)}). Discarding scraper results.`);
                triggerFallback = true;
            } else {
                finalResults = allScraperResults; // Trust the scraper's results
            }
        }
    } else {
        triggerFallback = true;
    ...
    if (triggerFallback) {
        ...
        try {
            ...
            finalResults = parsePriceApiResults(downloadedJobs);
        } catch (error) {
