// server.js (FINAL V12, with "Entity-First" Architecture & Intelligent Sort)
...
// --- File System and State Functions ---
...
app.post('/submit-results', async (req, res) => {
    ...
    if (intent === 'FIND_MAIN_PRODUCT') {
        // We no longer filter here. We use the score for sorting.
        console.log(`[Scoring] Scores calculated for main product search.`);
    } else {
        console.log(`[Scoring] Skipping negative scoring for accessory-focused search.`);
    }
    ...
    if (processedResults.length > 0) {
        ...
        if (simplifiedQuery) {
            console.log(`[Retry] No results found. Suggesting simplified query: "${simplifiedQuery}"`);
...
app.post('/admin/set-permanent-message', async (req, res) => {
    ...
    liveState.permanentMessage = message;
    await updateLiveStateFile();
    console.log(`ADMIN ACTION: Permanent message has been set.`);
    res.json({ message: `Permanent message has been set.` });
});
app.post('/admin/clear-permanent-message', async (req, res) => {
    const { code } = req.body;
    if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' });
    liveState.permanentMessage = "";
    await updateLiveStateFile();
    console.log(`ADMIN ACTION: Permanent message has been cleared.`);
    res.json({ message: `Permanent message has been cleared.` });
});
app.post('/admin/flash-message', async (req, res) => {
    const { code, message } = req.body;
    if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' });
    if (!message) return res.status(400).json({ error: 'Message text is required.' });
    liveState.flashMessage = { text: message, timestamp: Date.now() };
    await updateLiveStateFile();
...
// These are hoisted for clarity but defined here for brevity in the main code
Object.assign(global, {
    ...
    GENERIC_ACCESSORY_BRANDS: ['uag', 'otterbox', 'spigen', 'zagg', 'mophie', 'lifeproof', 'incipio', 'tech21', 'cygnett', 'efm', '3sixt', 'belkin', 'smallrig', 'stm', 'dbramante1928', 'razer', 'hyperx', 'logitech', 'steelseries', 'turtle beach', 'astro', 'powera', '8bitdo', 'gamesir'],
    GENERIC_ACCESSORY_KEYWORDS: ['case', 'cover', 'protector', 'charger', 'cable', 'adapter', 'stand', 'mount', 'holder', 'strap', 'band', 'replacement', 'skin', 'film', 'glass', 's-pen', 'spen', 'stylus', 'prismshield', 'kevlar', 'folio', 'holster', 'pouch', 'sleeve', 'wallet', 'battery pack', 'kit', 'cage', 'lens', 'tripod', 'gimbal', 'silicone', 'leather', 'magsafe', 'rugged', 'remote', 'remote control'],
    INTENT_ACCESSORY_KEYWORDS: ['case', 'cover', 'protector', 'charger', 'cable', 'adapter', 'stand', 'mount', 'holder', 'strap', 'band', 'replacement', 'skin', 'film', 'glass', 's-pen', 'spen', 'stylus', 'battery', 'kit', 'cage', 'lens', 'tripod', 'gimbal', 'magsafe', 'remote', 'remote control', 'controller', 'headset'],
    getQueryIntent: getQueryIntent,
    filterByQueryStrictness: filterByQueryStrictness,
    calculateScore: calculateScore,
    detectItemCondition: detectItemCondition,
    extractColorFromTitle: extractColorFromTitle,
    parsePythonResults: parsePythonResults,
    enrichResultsWithImages: enrichResultsWithImages,
    fetchImageForQuery: fetchImageForQuery,
    simplifyQuery: simplifyQuery,
    fetchOzbargainBackup: fetchOzbargainBackup
});
