// server.js (FINAL - With Correct JSON Parsing and Fallback)
...
function parsePythonResults(results) { return results.map(item => { const fullText = item.title; const priceMatch = fullText.match(/\$\s?[\d,]+(\.\d{2})?/); const priceString = priceMatch ? priceMatch : null; const price = priceMatch ? parseFloat(priceString.replace(/[^0-9.]/g, '')) : null; const words = fullText.split(' '); const store = words; if (!price) return null; return { title: fullText, price: price, price_string: priceString, store: store, url: item.url || '#' }; }).filter(Boolean); }

// --- MODIFICATION: This is the new, correct parser based on your working code and JSON sample ---
function parsePriceApiResults(downloadedJobs) {
    ...
    for (const jobData of downloadedJobs) {
        ...
        const jobResult = jobData.results;

        let mapped = [];
        // Logic for Amazon-style results
        if (topic === 'product_and_offers' && jobResult.content) {
            const content = jobResult.content;
            if (content.name && content.offers && Array.isArray(content.offers) && content.offers.length > 0) {
                // If there are multiple offers, map each one
                mapped = content.offers.map(offer => ({
                    title: content.name,
                    price: parseFloat(offer.price),
                    price_string: offer.price ? `$${parseFloat(offer.price).toFixed(2)}` : 'N/A',
                    url: offer.url,
                    image: content.image_url,
                    store: offer.shop_name || sourceName
                }));
            } else if (content.name && content.buybox) {
                // Fallback to the main buybox if offers array is empty
                const offer = content.buybox;
                 if (offer.min_price) {
                    mapped.push({
                        title: content.name,
                        price: parseFloat(offer.min_price),
                        price_string: `$${parseFloat(offer.min_price).toFixed(2)}`,
                        url: content.url,
                        image: content.image_url,
                        store: offer.shop_name || sourceName
                    });
        ...
        // Logic for Google Shopping/Ebay style results from your old code
            ...
            mapped = searchResults.map(item => {
                ...
                if (item.price) {
                    ...
                    price_string = item.price_string || `$${price.toFixed(2)}`;
