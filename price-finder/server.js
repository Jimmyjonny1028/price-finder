// server.js (FINAL - Scraper + API Backup Hybrid)
...
// --- TRANSPLANTED from your old API code ---
...
// --- HELPER FUNCTIONS for filtering (used by both scraper and API results) ---
...
// --- API CALLING FUNCTIONS (TRANSPLANTED from your old code) ---
...
async function searchPriceApiCom(query) {
    ...
    try {
        ...
        console.log(`[Backup API] Jobs submitted. Waiting 30s for processing...`);
        await wait(30000);
...
app.post('/report-failure', async (req, res) => {
    ...
    res.status(200).send('Failure reported. Backup API initiated.'); // Respond immediately
