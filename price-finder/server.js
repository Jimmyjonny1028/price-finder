...
// ### NEW: Smarter function to find the main product instead of just removing accessories ###
function filterForMainProduct(results, query) {
    const queryWords = query.toLowerCase().split(' ').filter(w => w.length > 2 && isNaN(w));
    if (queryWords.length === 0) return results; // Not enough info in query to filter
    ...
    // Keep results that match at least 2 of the main query words, or half if the query is short
    const requiredScore = Math.min(2, Math.max(1, Math.floor(queryWords.length / 2)));
    return scoredResults.filter(item => item.score >= requiredScore);
}
