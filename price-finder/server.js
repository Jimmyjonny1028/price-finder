// server.js (FINAL - With Correct Job Queue and All Features)
...
function triggerNextJob() {
    if (isWorkerBusy || jobQueue.length === 0 || !workerSocket) {
        return; // Do nothing if worker is busy, queue is empty, or no worker
    }
    isWorkerBusy = true;
...
wss.on('connection', (ws, req) => {
    ...
    workerSocket = ws;
    isWorkerBusy = false;
    triggerNextJob(); // Immediately check for any pending jobs
...
// =================================================================
// HELPER FUNCTIONS (for filtering data from the Python scraper)
// =================================================================
...
function parsePythonResults(results) {
    return results.map(item => {
        ...
        return {
            ...
            image: 'https://via.placeholder.com/150/E2E8F0/A0AEC0?text=Image+N/A', // Static placeholder
            url: '#'
        };
    }).filter(Boolean);
}
...
app.get('/search', async (req, res) => {
    ...
    if (workerSocket) {
        // --- THIS IS THE FIX FOR JOB SPAMMING ---
        if (!jobQueue.includes(query) && (!isWorkerBusy || workerSocket.currentJob !== query)) {
            jobQueue.push(query);
...
app.post('/submit-results', (req, res) => {
    ...
    console.log(`Received ${results.length} raw text results for "${query}" from Python worker.`);
