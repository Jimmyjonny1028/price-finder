// server.js (FINAL - With Correct Parsing and Type Handling)
...
app.post('/submit-results', async (req, res) => {
    ...
    if (finalFilteredScraperResults.length > 0) {
        ...
        try {
            ...
            let allApiResults = parsePriceApiResults(downloadedJobs).map(item => ({ ...item, price: parseFloat(String(item.price_string || item.price).replace(/[^0-9.]/g, '')), image: formatImageUrl(item.image) })).filter(item => !isNaN(item.price));
