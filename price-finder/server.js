...
// ### NEW: A generic filter to ensure results match the query. This fixes the ReferenceError. ###
const filterResultsByQuery = (results, query) => {
    const queryLower = query.toLowerCase();
    const queryTerms = queryLower.split(' ').filter(w => w.length > 1);
    if (queryTerms.length === 0) return results;
    return results.filter(item => {
        const itemTitle = item.title.toLowerCase();
        return queryTerms.every(term => itemTitle.includes(term));
    });
};

// ### NEW: A smarter filter to remove cheap outliers based on price gaps. ###
function filterPriceOutliers(results) {
    if (results.length < 5) return results; // Not enough data to find outliers
    const sortedResults = [...results].sort((a, b) => a.price - b.price);
    let cutOffIndex = 0;
    const GAP_MULTIPLIER = 2.5; // A price jump of 2.5x indicates a different class of product
    const MIN_PRICE_FOR_JUMP = 100; // The jump must be to a price over $100

    for (let i = 0; i < sortedResults.length - 1; i++) {
        const currentPrice = sortedResults[i].price;
        const nextPrice = sortedResults[i+1].price;
        if (nextPrice > currentPrice * GAP_MULTIPLIER && nextPrice > MIN_PRICE_FOR_JUMP) {
            cutOffIndex = i + 1;
        }
    }

    if (cutOffIndex > 0) {
        console.log(`[Price Outlier Filter] Discarding ${cutOffIndex} items cheaper than the main product cluster.`);
        return sortedResults.slice(cutOffIndex);
    }
