// server.js (FINAL - Production Ready with Public API)
...
const apiJobs = new Map(); // Cache for API job results
...
async function loadApiKeys() { try { await fs.access(API_KEYS_PATH); const data = await fs.readFile(API_KEYS_PATH, 'utf8'); const keysArray = JSON.parse(data); apiKeys = new Set(keysArray); console.log(`✅ Loaded ${apiKeys.size} API keys.`); } catch (e) { console.log("API key file not found, starting fresh."); apiKeys = new Set(); } }
async function saveApiKey(hashedKey) { apiKeys.add(hashedKey); try { await fs.writeFile(API_KEYS_PATH, JSON.stringify([...apiKeys]), 'utf8'); } catch(e) { console.error("Could not save API key:", e); } }
...
function dispatchJob() { if (isQueueProcessingPaused || !workerSocket || jobQueue.length === 0) return; const nextQuery = jobQueue.shift(); workerSocket.send(JSON.stringify({ type: 'NEW_JOB', query: nextQuery.query, jobId: nextQuery.jobId })); }
...
app.post('/submit-results', async (req, res) => { const { secret, query, results, jobId } = req.body; if (secret !== SERVER_SIDE_SECRET) { return res.status(403).send('Forbidden'); } if (!query || !results) { return res.status(400).send('Bad Request: Missing query or results.'); } res.status(200).send('Results received. Processing now.'); let allScraperResults = parsePythonResults(results); const isComponentOrAccessory = detectSearchIntent(query); let finalResults = []; let triggerFallback = false; if (allScraperResults.length > 0) { if (isComponentOrAccessory) { finalResults = filterAndRankResults(allScraperResults, query); } else { const maxPrice = Math.max(...allScraperResults.map(item => item.price)); if (maxPrice < MAIN_PRODUCT_PRICE_THRESHOLD) { triggerFallback = true; } else { finalResults = filterPriceOutliers(allScraperResults); } } } if (finalResults.length === 0) { triggerFallback = true; } if (triggerFallback) { try { const downloadedJobs = await searchPriceApiCom(query); finalResults = filterAndRankResults(parsePriceApiResults(downloadedJobs), query); } catch (error) { finalResults = []; } } const resultsWithImages = await enrichResultsWithImages(finalResults, query); const sortedResults = resultsWithImages.sort((a, b) => a.price - b.price).map(item => ({ ...item, condition: detectItemCondition(item.title) })); if (jobId) { apiJobs.set(jobId, { status: 'complete', results: sortedResults }); } else { searchCache.set(query.toLowerCase(), { results: sortedResults, timestamp: Date.now() }); } });

// --- NEW: PUBLIC API ENDPOINTS ---
const apiAuthMiddleware = (req, res, next) => {
    const providedApiKey = req.headers['x-api-key'];
    if (!providedApiKey) return res.status(401).json({ error: 'API key is missing.' });
    const hashedKey = createHash('sha256').update(providedApiKey).digest('hex');
    if (!apiKeys.has(hashedKey)) return res.status(403).json({ error: 'Invalid API key.' });
    next();
};
...
const keyRequestLimiter = rateLimit({
    windowMs: 60 * 60 * 1000,
    max: 5,
    message: 'Too many API keys requested from this IP, please try again after an hour',
    standardHeaders: true,
    legacyHeaders: false,
});

app.post('/api/v1/request-key', keyRequestLimiter, async (req, res) => {
    const newKey = `df_` + randomUUID();
    const hashedKey = createHash('sha256').update(newKey).digest('hex');
    await saveApiKey(hashedKey);
    console.log("New API key generated.");
    res.status(201).json({ 
        message: "API Key generated successfully! Please save this key, it will not be shown again.",
        apiKey: newKey 
    });
});

app.post('/api/v1/search', apiAuthMiddleware, apiRequestLimiter, (req, res) => {
    const { query } = req.body;
    if (!query) return res.status(400).json({ error: 'Search query is required' });

    const jobId = randomUUID();
    apiJobs.set(jobId, { status: 'pending', results: [] });

    if (workerSocket) {
        jobQueue.push({ query, jobId });
        dispatchJob();
    } else {
...
app.get('/api/v1/results/:job_id', apiAuthMiddleware, (req, res) => {
    const { job_id } = req.params;
    if (!apiJobs.has(job_id)) {
        return res.status(404).json({ error: 'Job ID not found.' });
    }
    const job = apiJobs.get(job_id);
    if (job.status === 'pending') {
        return res.status(202).json({ status: 'pending', message: 'Results are not ready yet.' });
    }
    apiJobs.delete(job_id);
    res.status(200).json({ status: 'complete', results: job.results });
});
