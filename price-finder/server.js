...
function dispatchJob() {
    // ### MODIFIED: Check if queue processing is paused ###
...
async function fetchImageForQuery(query) { const cacheKey = query.toLowerCase(); if (imageCache.has(cacheKey)) { return imageCache.get(cacheKey); } const placeholder = 'https://via.placeholder.com/150/E2E8F0/A0AEC0?text=Image+N/A'; if (!GOOGLE_API_KEY || !GOOGLE_CSE_ID) { return placeholder; } try { const url = `https://www.googleapis.com/customsearch/v1`; const params = { key: GOOGLE_API_KEY, cx: GOOGLE_CSE_ID, q: query, searchType: 'image', num: 1 }; const response = await axios.get(url, { params }); const imageUrl = response.data.items?.[0]?.link || placeholder; imageCache.set(cacheKey, imageUrl); await saveImageCacheToFile(); return imageUrl; } catch (error) { console.error(`[FATAL] Google Image Search request failed for query: "${query}"`); if (error.response) { console.error('Error Data:', JSON.stringify(error.response.data, null, 2)); } else { console.error('Error Message:', error.message); } return placeholder; } }
async function enrichResultsWithImages(results, baseQuery) { if (results.length === 0) return results; console.log(`Enriching ${results.length} results with images...`); const defaultImageUrl = await fetchImageForQuery(baseQuery); const imageEnrichmentPromises = results.map(async (result) => { const color = extractColorFromTitle(result.title); let imageUrl = defaultImageUrl; if (color) { const specificQuery = `${baseQuery} ${color}`; imageUrl = await fetchImageForQuery(specificQuery); } result.image = imageUrl; return result; }); return Promise.all(imageEnrichmentPromises); }
...
// --- ADMIN ROUTES ---
// ### MODIFIED: Main data endpoint now includes more data ###
app.post('/admin/traffic-data', (req, res) => {
    ...
    res.json({
        ...
        workerStatus: workerSocket ? 'Connected' : 'Disconnected',
        activeJobs: Array.from(workerActiveJobs),
        jobQueue: jobQueue,
        isQueuePaused: isQueueProcessingPaused,
...
// ### NEW: More Admin Powers ###
app.post('/admin/toggle-queue', (req, res) => {
    const { code } = req.body;
    if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' });
    isQueueProcessingPaused = !isQueueProcessingPaused;
    const message = `Job queue processing has been ${isQueueProcessingPaused ? 'PAUSED' : 'RESUMED'}.`;
    console.log(`ADMIN ACTION: ${message}`);
    if (!isQueueProcessingPaused) dispatchJob(); // Attempt to dispatch a job immediately on resume
    res.json({ isQueuePaused: isQueueProcessingPaused, message });
});
...
app.post('/admin/clear-queue', (req, res) => {
    ...
    console.log("ADMIN ACTION: Job queue has been cleared.");
    res.json({ message: 'Job queue has been cleared successfully.' });
});

app.post('/admin/disconnect-worker', (req, res) => {
    const { code } = req.body;
    if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' });
    if (workerSocket) {
        workerSocket.close();
        console.log("ADMIN ACTION: Forcibly disconnected the worker.");
        res.json({ message: 'Worker has been disconnected.' });
    } else {
        res.status(404).json({ message: 'No worker is currently connected.' });
    }
});

app.post('/admin/clear-image-cache', async (req, res) => {
    const { code } = req.body;
    if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' });
    imageCache.clear();
    await saveImageCacheToFile(); // Overwrite the file with an empty cache
    console.log("ADMIN ACTION: Permanent image cache has been cleared.");
    res.json({ message: 'The permanent image cache has been cleared.' });
});
...
// Function to start the server after loading the cache
