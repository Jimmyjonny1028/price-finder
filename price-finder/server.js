...
const USER_ONLINE_TIMEOUT_MS = 60 * 1000;
let isQueueProcessingPaused = false;
let currentTheme = 'default';
let limit; // To be initialized in startServer
...
// ### NEW: Dynamic price filter function ###
function filterByMeanPrice(results, thresholdFactor = 0.5) {
    if (results.length < 5) { // Don't filter if there aren't enough results for a meaningful average
        return results;
    }
    const prices = results.map(item => item.price).filter(price => price > 0);
    if (prices.length < 3) {
        return results;
    }
    const sum = prices.reduce((a, b) => a + b, 0);
    const mean = sum / prices.length;
    const priceThreshold = mean * thresholdFactor;
    console.log(`[Dynamic Filter] Mean price: $${mean.toFixed(2)}, Filtering items below: $${priceThreshold.toFixed(2)}`);
...
app.post('/submit-results', async (req, res) => { const { secret, query, results } = req.body; if (secret !== SERVER_SIDE_SECRET) { return res.status(403).send('Forbidden'); } if (!query || !results) { return res.status(400).send('Bad Request: Missing query or results.'); } res.status(200).send('Results received. Processing now.'); let allScraperResults = parsePythonResults(results); const isAccessorySearch = detectSearchIntent(query); let finalFilteredScraperResults; if (isAccessorySearch) { finalFilteredScraperResults = filterResultsByQuery(allScraperResults, query); } else { 
        // ### MODIFIED: Using the new dynamic filter instead of the static one ###
        const priceFiltered = filterByMeanPrice(allScraperResults);
    ...
    } if (finalFilteredScraperResults.length > 0) { const resultsWithImages = await enrichResultsWithImages(finalFilteredScraperResults, query); const sortedResults = resultsWithImages.sort((a, b) => a.price - b.price).map(item => ({ ...item, condition: detectItemCondition(item.title) })); searchCache.set(query.toLowerCase(), { results: sortedResults, timestamp: Date.now() }); } else { try { const downloadedJobs = await searchPriceApiCom(query); let allApiResults = parsePriceApiResults(downloadedJobs); let finalFilteredApiResults; if (isAccessorySearch) { finalFilteredApiResults = filterResultsByQuery(allApiResults, query); } else { 
        // Using the dynamic filter for the backup API results as well
        const priceFiltered = filterByMeanPrice(allApiResults);
        const accessoryFiltered = filterForIrrelevantAccessories(priceFiltered);
        finalFilteredApiResults = filterForMainDevice(accessoryFiltered);
    } const resultsWithImages = await enrichResultsWithImages(finalFilteredApiResults, query); const sortedResults = resultsWithImages.sort((a, b) => a.price - b.price).map(item => ({ ...item, condition: detectItemCondition(item.title) })); searchCache.set(query.toLowerCase(), { results: sortedResults, timestamp: Date.now() }); } catch (error) { console.error(`[Backup API] A critical error occurred during the fallback for "${query}":`, error); searchCache.set(query.toLowerCase(), { results: [], timestamp: Date.now() }); } } });

// Public API routes
app.get('/api/get-theme', (req, res) => { res.json({ theme: currentTheme }); });
app.post('/api/ping', (req, res) => { const { sessionID } = req.body; if (!sessionID) return res.status(400).send(); if (onlineUsers.has(sessionID)) { clearTimeout(onlineUsers.get(sessionID)); } const timeoutID = setTimeout(() => { onlineUsers.delete(sessionID); }, USER_ONLINE_TIMEOUT_MS); onlineUsers.set(sessionID, timeoutID); res.status(200).json({ status: 'ok' }); });

// Admin Routes
app.post('/admin/traffic-data', (req, res) => { const { code } = req.body; if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' }); res.json({ totalSearches: trafficLog.totalSearches, uniqueVisitors: trafficLog.uniqueVisitors.size, searchHistory: trafficLog.searchHistory, isServiceDisabled: isMaintenanceModeEnabled, workerStatus: workerSocket ? 'Connected' : 'Disconnected', activeJobs: Array.from(workerActiveJobs), jobQueue: jobQueue, isQueuePaused: isQueueProcessingPaused, imageCacheSize: imageCache.size, currentTheme: currentTheme, onlineUsers: onlineUsers.size }); });
...
app.post('/admin/set-theme', (req, res) => { const { code, theme } = req.body; if (!code || code !== ADMIN_CODE) return res.status(403).json({ error: 'Forbidden' }); const validThemes = ['default', 'dark', 'retro']; if (theme && validThemes.includes(theme)) { currentTheme = theme; console.log(`ADMIN ACTION: Global theme changed to "${theme}".`); res.json({ message: `Theme changed to ${theme}.` }); } else { res.status(400).json({ error: 'Invalid theme specified.' }); } });
